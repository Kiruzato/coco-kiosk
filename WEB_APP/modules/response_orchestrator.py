...
from WEB_APP.modules.retrieval_validator import (
    extract_query_terms,
...
from WEB_APP.modules.confidence_scorer import compute_confidence_score, ConfidenceLevel
from WEB_APP.modules.entity_extractors import (
...
try:
    from WEB_APP.modules.math_engine import is_calculable_expression, try_calculate
...
class ResponseOrchestrator:
    ...
    @staticmethod
    def _get_fusion_label_visible() -> bool:
        """Read fusion_label_visible from app config (lazy import to avoid circular dependency)."""
        try:
            from WEB_APP.app import fusion_label_visible
            return fusion_label_visible
        except ImportError:
            return True
    ...
    def process_query(
        ...
        mode = self._determine_response_mode(
            governance, retrieval, extraction
        )
    ...
    async def process_query_streaming(
            ...
            if success and answer:
                ...
                yield {
                    ...
                    "data": {
                        ...
                        "fusion_mode": "linear",
                        "fusion_label_visible": self._get_fusion_label_visible(),
                        "debug_info": {
                            "response_mode": "general",
        ...
        yield {
            ...
            "data": {
                ...
                "fusion_mode": retrieval.fusion_method,
                "fusion_label_visible": self._get_fusion_label_visible(),
                "debug_info": {
                    "response_mode": mode.value,
    ...
    def _build_prompt_for_mode(
        ...
        """
        from WEB_APP.modules.query_analyzer import analyze_query, QueryType
    ...
    def _apply_governance(self, query: str) -> GovernanceResult:
        ...
        """
        from WEB_APP.modules.intent_classifier import classify_intent, is_directory_query, safety_check_general_mode
    ...
    def _perform_retrieval(self, query: str) -> RetrievalResult:
        ...
        # Read fusion config from app settings (lazy import to avoid circular dependency)
        _fusion_method = "linear"
        _rrf_k = 60
        try:
            from WEB_APP.app import fusion_method as _fm, rrf_k as _rk
            _fusion_method = _fm
            _rrf_k = _rk
        except ImportError:
            pass
        ...
        result.hybrid_scores, _ = combine_hybrid_scores(
            ...
            method=_fusion_method, rrf_k=_rrf_k
        )
        result.fusion_method = _fusion_method
